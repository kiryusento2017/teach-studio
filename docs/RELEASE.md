...
**这条跟本文件其它地方讲的「每行铺满」不是同一件事，技术原因也不同，别混。**

GitHub 的 Release 说明走 GFM，**单个换行会被渲染成真正的断行**（`<br>`），
不像标准 Markdown 那样把软换行当空格合并。所以源码里折到 70 字符，网页上
就在那儿断掉，右边空一大片——一段话被切成好几条短行。

```markdown
差（源码折行，网页上就是三条短行）：
    第一次装的人下 `teach-studio-Setup-v0.0.3.exe`，装法跟以前一样：
    **装到 D 盘之类的地方**，别装进 `C:\Program Files`（那里写不了文件），
    也别留在「下载」文件夹里。

好（一段一行，让 GitHub 自己排）：
    第一次装的人下 `teach-studio-Setup-v0.0.3.exe`，装法跟以前一样：**装到 D 盘之类的地方**，别装进 `C:\Program Files`（那里写不了文件），也别留在「下载」文件夹里。
```
...
🔴 **v0.0.2 就是这么发出去的**（2026-09-09 小蔡在 GitHub 上一眼看出来的：
「为什么没有铺满」）。同一份说明里还抄错了版本号——写成
`teach-studio-Setup-v0.1.2.exe`，那是本地版的号。两个错同源：
**照着本地版的说明改，却没改干净，也没在网页上看一眼渲染结果**。

发之前把说明贴进 GitHub 的 Release 编辑框预览一次，两类错一起现形。
